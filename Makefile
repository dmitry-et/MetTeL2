...
FO_GRAMMAR_FILE_NAME := $(FO_GRAMMAR_NAME).g4
...
$(PARSER_FILES): $(GRAMMAR_FILE)
	...
	@ $(JAVA) -jar $(ANTLR4_JAR) -no-listener -no-visitor $(ANTLR_OPTIONS) $(GRAMMAR_FILE) 1>$(ANTLR_PARSER_LOG_FILE) 2>&1 && rm -f $(TOKENS_FILES) && mv -f $(PARSER_FILES_G) $(PARSER_DIR)
